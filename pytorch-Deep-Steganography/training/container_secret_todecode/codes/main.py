...
COVER_DATA_DIR = '../../high-res-cover-images/'
SECRET_DATA_DIR = '../../secret_images/'
...
def save_result_pic(this_batch_size, originalLabelv, ContainerImg, secretLabelv, RevSecImg, epoch, i, save_path):
    if not opt.debug:
        ...
        #secretFrames = secretLabelv.resize_(this_batch_size, 3, 1760, 1440)
        #revSecFrames = RevSecImg.resize_(this_batch_size, 3, 1760, 1440)
        from torchvision import transforms
        trans = transforms.ToPILImage()
        tens = transforms.ToTensor()
        pwe = transforms.Compose([transforms.Resize((1436,1756))])
        RevSecImg = tens(pwe(trans(RevSecImg.cpu().squeeze(0)))).unsqueeze(0)
        #import pdb
        #pdb.set_trace()
        ...
        resultConName = '%s/Container_epoch%03d_batch%04d.png' % (save_path, epoch, i)
        resultSecName = '%s/Secret_epoch%03d_batch%04d.png' % (save_path, epoch, i)
        #vutils.save_image(resultImg, resultImgName, nrow=this_batch_size, padding=1, normalize=True)
        vutils.save_image(ContainerImg,resultConName)
        vutils.save_image(RevSecImg,resultSecName)
